...
GOOGLE_CLIENT_ID = "1017008845478-a7n6dfunau8qpeac9fmlnsvbuhdlasb8.apps.googleusercontent.com"
GOOGLE_CLIENT_SECRET = "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"
...
def books():
    ...
    for post in posts:
        base_url = 'https://robertsrandomreviews.com/post/'
        post_id = str(post.id)
        temp_url = base_url + post_id
        if post.author_name in book_dict:
            book_dict[post.author_name].append([post.title, temp_url])
        else:
            book_dict[post.author_name] = [[post.title, temp_url]]
